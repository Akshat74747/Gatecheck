 ...
 *
 * Pure function tests: no DB, no LLM, no GitHub.
 * Ported from Bugbash-main/server/src/__tests__/security.rules.test.ts
 */
import { describe, it, expect } from "vitest";
import { runAllRules, emptyInput } from "../lib/security/rules";
import type { RuleInput, RuleFile } from "../lib/security/rules/types";
...
jobs:
  build:
    ...
    expect(hit).toBeDefined();
    expect(hit!.suggestion).toContain("${{ github.event.pull_request.head.sha }}");
